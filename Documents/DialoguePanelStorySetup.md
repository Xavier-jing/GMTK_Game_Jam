...
`StoryPresenter.Awake()` 会隐藏 `Panel Root`。这是正常行为：剧情启动后，
`ShowDialogue()` 或 `ShowChoices()` 会重新显示它。

### 2.3 配置 StoryController

推荐把 `StoryController` 添加到 `Canvas`，因为当前 `GamePlay` 场景的
`Canvas` 已经带有 `UIInputHandler`，且 `DialoguePanel` 是它的子对象。
...
- `Story Controller`：建议显式指向 `Canvas` 上的 `StoryController`；
  留空时运行时会在场景中查找一个。
- `First Script Id`：填写
